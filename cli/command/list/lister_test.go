...
func (s *ListCmdTestSuite) TestListCmdRun() {
	procOne := procMetadata.Metadata{
		Name:        "one",
		Description: "proc one description",
	}
	procTwo := procMetadata.Metadata{
		Name:        "two",
		Description: "proc two description",
	}
	procList := []procMetadata.Metadata{procOne, procTwo}
...
func (s *ListCmdTestSuite) TestListCmdRunProctorDClientFailure() {
	s.mockProctorDClient.On("ListProcs").Return([]procMetadata.Metadata{}, errors.New("Error!!!\nUnknown Error.")).Once()
